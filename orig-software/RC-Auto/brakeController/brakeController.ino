...
//unsigned char drv_clutch = 0; not used
...
// rear right : zero pos = 1800
// rear right : max pos = 1100
  
// rear left : zero pos = 800
// rear left : max pos = 1200
...
void setup(){
  ...
  //Light the led on the Arduino board (B7) to see that the program is running
  //PORTB |= (1<<PB7);
...
void update_brakes()
{ 
    /*int flw_pulseFixed = ((int)flw_pulsecount + (int)flw_pulsecountPrev) / 2;
    flw_pulsecountPrev = flw_pulsecount;
    
    int frw_pulseFixed = ((int)frw_pulsecount + (int)frw_pulsecountPrev) / 2;
    frw_pulsecountPrev = frw_pulsecount;
    
    int rlw_pulseFixed = ((int)rlw_pulsecount + (int)rlw_pulsecountPrev) / 2;
    rlw_pulsecountPrev = rlw_pulsecount;
    
    int rrw_pulseFixed = ((int)rrw_pulsecount + (int)rrw_pulsecountPrev) / 2;
    rrw_pulsecountPrev = rrw_pulsecount;
  */
    ...
    /*
    double flw_a = (1.227118 * (flw_pulseFixed - flw_ws_prev)) / 0.01;
    double frw_a = (1.227118 * (frw_pulseFixed - frw_ws_prev)) / 0.01;
    double rlw_a = (1.227118 * (rlw_pulseFixed - rlw_ws_prev)) / 0.05;
    double rrw_a = (1.227118 * (rrw_pulseFixed - rrw_ws_prev)) / 0.05; // update correct time
    */
    ...
    /*
    flw_ws_prev = flw_pulseFixed;
    frw_ws_prev = frw_pulseFixed;
    rlw_ws_prev = rlw_pulseFixed;
    rrw_ws_prev = rrw_pulseFixed;
    */
  if(esp_on)
  { 
...
void read_serial1_byte(unsigned char c)
{
  ...
  else if(serial1_cur_msgId == 0x32) // message from pc
  {
    ...
    if(serial1_reading_byte_num == 3)
    {
      ...
      Serial.println(_param_buff[0]);
      Serial.print("value1: ");
      Serial.println(_param_buff[1]);
      Serial.print("value2: ");
      Serial.println(_param_buff[2]);*/
